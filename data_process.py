...
for i in range(1, int(np.max(train_01_nor[:, 0])) + 1):  
    ...
    ind = ind[0]
    data_temp = train_01_nor[ind, :]  #data_temp is the data of the i-th engine
    for j in range(len(data_temp) - window_Size + 1): 
        trainX.append(data_temp[j:j + window_Size, 2:].tolist()) 
        train_RUL = len(data_temp) - window_Size - j  #RUL of the j-th window
        train_bu = RUL_max - train_RUL  # Remaining useful life
...
sio.savemat('F002_window_size_trainX.mat', {"train1X": trainX})
sio.savemat('F002_window_size_trainY.mat', {"train1Y": trainY})
sio.savemat('F002_window_size_testX.mat', {"test1X": testX})
sio.savemat('F002_window_size_testY.mat', {"test1Y": testY})
...
trainX = sio.loadmat('F001_window_size_trainX.mat')['train1X']
testX =  sio.loadmat('F001_window_size_testX.mat')['test1X']

print('the shape of trainX is:', trainX.shape)
